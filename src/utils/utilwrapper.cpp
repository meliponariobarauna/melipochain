...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Meliponariod
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Meliponariod
    // Mac and Unix: ~/.meliponario
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "MeliponariodCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Meliponariod";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".meliponario-cold";        
    }
    return pathRet / ".meliponario";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"meliponario",".conf") == 0)
    {
...
int mc_Params::ReadConfig(const char *network_name)
{
    ...
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"meliponario",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_BitchainParams::SetProtocolGlobals()
{
...
int mc_BitchainParams::SetGlobals()
{
...
int mc_BitchainParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string BitchainServerAddress(bool check_external_ip)
{
