...
#include "meliponario/meliponario.h"                                              // MCHN
...
/* MCHN START */
static boost::filesystem::path pathCachedMeliponariod;
static boost::filesystem::path pathCachedMeliponariodLog;
...
/* MCHN START */
    ...
    pathCachedMeliponariod=fs::path(string(mc_gState->m_Params->DataDir()));
    fs::path &path =pathCachedMeliponariod;
    return path;
...
/* MCHN START */
    fs::path &path =pathCachedMeliponariodLog;
    if (!path.empty())
        return path;
    path=fs::path(string(mc_gState->m_Params->DataDir(2,1)));
    return pathCachedMeliponariodLog;
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "meliponario.pid"));
