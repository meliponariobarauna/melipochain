...
int mc_MeliponariodFilterParams::Zero()
{
    m_MaxShownData=-1;
...
int mc_MeliponariodFilterParams::Init()
{
    Zero();
...
int mc_MeliponariodFilterParams::Close()
{
    mc_gState->m_Compatibility=m_Compatibility;    
    return MC_ERR_NOERROR;
}

int mc_MeliponariodFilterParams::Destroy()
{
    return Zero();
}
...
int mc_MeliponariodLibrary::Zero()
{
...
int mc_MeliponariodLibrary::Destroy()
{
    Zero();
...
int mc_MeliponariodLibrary::Initialize(const unsigned char* short_txid,uint32_t index)
{
    return MC_ERR_NOERROR;    
}

int mc_MeliponariodFilter::Zero()
{
...
int mc_MeliponariodFilter::Destroy()
{
    if(m_CachedWorker)
    {
...
int mc_MeliponariodFilter::Initialize(const unsigned char* short_txid)
{
    size_t value_size;
    unsigned char *ptr;
    ...
    if(ptr)
    {
        m_FilterCodeRow=pMeliponariodFilterEngine->m_CodeLibrary->GetNumElements();
        pMeliponariodFilterEngine->m_CodeLibrary->AddElement();
        pMeliponariodFilterEngine->m_CodeLibrary->SetData(ptr,value_size);
        pMeliponariodFilterEngine->m_CodeLibrary->SetData(&ntc,1);        
    }                                    
    else
    {
        m_FilterCodeRow=pMeliponariodFilterEngine->m_CodeLibrary->GetNumElements();
...
bool mc_MeliponariodFilter::HasRelevantEntity(set <uint160>& sRelevantEntities)
{
...
int mc_MeliponariodFilterEngine::Zero()
{
    m_Filters.clear();
    m_TxID=0;
    m_EntityTxID=0;
...
int mc_MeliponariodFilterEngine::Destroy()
{
    if(m_Semaphore)
    {
...
void mc_MeliponariodFilterEngine::Lock(int write_mode)
{        
...
void mc_MeliponariodFilterEngine::UnLock()
{    
    m_LockedBy=0;
    __US_SemPost(m_Semaphore);
}

int mc_MeliponariodFilterEngine::InFilter()
{    
...
int mc_MeliponariodFilterEngine::GetAcceptTimeout()
{    
    return GetArg("-acceptfiltertimeout",DEFAULT_ACCEPT_FILTER_TIMEOUT);
}

int mc_MeliponariodFilterEngine::GetSendTimeout()
{    
...
int mc_MeliponariodFilterEngine::SetTimeout(int timeout)
{
    Lock(1);
...
int mc_MeliponariodFilterEngine::SetTimeoutInternal(int timeout)
{
    for(int i=0;i<(int)m_Filters.size();i++)
    {
        mc_Filter *worker=*(mc_Filter **)m_Workers->GetRow(i);
...
uint160 mc_MeliponariodFilterEngine::ActiveUpdateID(uint160 hash)
{
    map<uint160,mc_MeliponariodLibrary>::iterator it=m_Libraries.find(hash);
...
int mc_MeliponariodFilterEngine::LoadLibrary(uint160 hash,bool *modified)
{
    int count;
    size_t value_size;
    unsigned char *ptr;
    ...
    mc_MeliponariodLibrary library[2];
    ...
    for(int i=0;i<count;i++)
    {
        ...
        map<uint160,mc_MeliponariodLibrary>::iterator it=m_Libraries.find(library[i].m_Hash);
        if (it == m_Libraries.end())
        {
            if(i == 0)
            {
...
int mc_MeliponariodFilterEngine::CheckLibraries(set<uint160>* lpAffectedLibraries, int for_block)
{
    Lock(1);
...
int mc_MeliponariodFilterEngine::CheckLibrariesInternal(set<uint160>* lpAffectedLibraries, int for_block)
{
    int err;
    bool modified;
    map<uint160,mc_MeliponariodLibrary>::iterator mit;
    map<uint160,mc_MeliponariodLibrary>::iterator lit;    
...
mc_Filter *mc_MeliponariodFilterEngine::StreamFilterWorker(int row,bool *modified)
{
    int err; 
    char *code;
    size_t code_size;
    ...
    *modified=false;
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        ...
        map<uint160,mc_MeliponariodLibrary>::iterator it=m_Libraries.find(mc_LibraryIDForUpdate(m_Filters[row].m_Libraries[i]));
        if(it == m_Libraries.end())
        {
...
int mc_MeliponariodFilterEngine::RebuildFilter(int row,int for_block)
{
    int err; 
    ...
    for (unsigned int i=0;i<m_Filters[row].m_Libraries.size();i++) 
    {
        map<uint160,mc_MeliponariodLibrary>::iterator it=m_Libraries.find(m_Filters[row].m_Libraries[i]);
        if (it != m_Libraries.end())
        {
...
int mc_MeliponariodFilterEngine::AddFilter(const unsigned char* short_txid,int for_block)
{
    Lock(1);
...
int mc_MeliponariodFilterEngine::AddFilterInternal(const unsigned char* short_txid,int for_block)
{    
    int err;
    mc_MeliponariodFilter filter;
...
int mc_MeliponariodFilterEngine::Reset(int block,int for_block)
{
    Lock(1);
...
int mc_MeliponariodFilterEngine::ResetInternal(int block,int for_block)
{
    int filter_block;
    int err;
...
int mc_MeliponariodFilterEngine::NoStreamFilters()
{
    int ret=1;
...
int mc_MeliponariodFilterEngine::RunStreamFilters(const CTransaction& tx,int vout, unsigned char *stream_short_txid,int block,int offset,std::string &strResult,mc_MeliponariodFilter **lppFilter,int *applied)            
{
    if(mc_gState->m_Features->StreamFilters() == 0)
    {
        return MC_ERR_NOERROR;
...
int mc_MeliponariodFilterEngine::RunTxFilters(const CTransaction& tx,std::set <uint160>& sRelevantEntities,std::string &strResult,mc_MeliponariodFilter **lppFilter,int *applied,bool only_once)
{    
    Lock(0);
...
int mc_MeliponariodFilterEngine::RunFilter(const CTransaction& tx,mc_Filter *filter,std::string &strResult)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
int mc_MeliponariodFilterEngine::RunFilterWithCallbackLog(const CTransaction& tx,int vout,uint256 stream_txid,mc_Filter *filter,std::string &strResult, json_spirit::Array& callbacks)
{
    Lock(0);
    int err=MC_ERR_NOERROR;
    m_Tx=tx;
...
void mc_MeliponariodFilterEngine::SetCallbackNames()
{
    Lock(1);
...
void mc_MeliponariodFilterEngine::SetCallbackNamesInternal()
{
    m_CallbackNames.clear();
...
int mc_MeliponariodFilterEngine::Initialize()
{
    mc_Buffer *filters;
    unsigned char *txid;
    int err=MC_ERR_NOERROR;
