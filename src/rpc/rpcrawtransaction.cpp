 ...
 */ 
    ...
    {
//        if(pMeliponariodFilterEngine->m_TxID != 0)
...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepMeliponariodTransaction
...
int VerifyNewTxForStreamFilters(const CTransaction& tx,std::string &strResult,mc_MeliponariodFilter **lppFilter,int *applied)            
{
    ...
    if(pMeliponariodFilterEngine->NoStreamFilters())
    {
        return MC_ERR_NOERROR;
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) 
    {
        ...
        if(result.type() == obj_type)
        {
            ...
            int err=pMeliponariodFilterEngine->RunStreamFilters(tx,i,(unsigned char*)&hash+MC_AST_SHORT_TXID_OFFSET,-1,0, 
...
//        throw runtime_error("Help message not found\n");
    ...
    if(pMeliponariodFilterEngine->m_Vout < 0)
    {
    ...
    if(pMeliponariodFilterEngine->m_Vout >= (int)pMeliponariodFilterEngine->m_Tx.vout.size())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vout out of range");                                            
    }
    Value result=DataItemEntry(pMeliponariodFilterEngine->m_Tx,pMeliponariodFilterEngine->m_Vout,streams_already_seen, 0x0E00);
...
//    result.get_obj().push_back(Pair("txid", pMeliponariodFilterEngine->m_Tx.GetHash().GetHex()));
    result.get_obj().push_back(Pair("vout", pMeliponariodFilterEngine->m_Vout));
...
Value getfilterstream(const Array& params, bool fHelp)
{
    ...
    Value result=StreamEntry((unsigned char*)&(pMeliponariodFilterEngine->m_EntityTxID),0x0002);
...
//    if(pMeliponariodFilterEngine->m_TxID != 0)
    if(pMeliponariodFilterEngine->InFilter)
    {
        ...
        tx=pMeliponariodFilterEngine->m_Tx;
    }
    else
    {
        if(params.size() == 0)
...
*/    
    Object result;
    TxToJSON(pMeliponariodFilterEngine->m_Tx, 0, result);
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(ParseBitchainTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(ParseBitchainTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=meliponario");
    }    
