...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need accounts, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
        }            
...
Value listaddresses(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "API is not supported with this wallet version. To get this functionality, run \"meliponariod -walletdbversion=2 -rescan\" ");        
    }   
...
Value txoutdata_operation(int rpc_slot,const Array& params,int fHan,int *errCode,string *strError)
{
 ...
 */ 
            ...
            for (int e = 0; e < tmpscript->GetNumElements(); e++)
            {
                ...
                if(tmpscript->GetRawData(&ptr,&size) == 0)      
                {
                    if(elem)
                    {
                        ...
                        *strError="This output has more than one data item";                                        
                        return 0;
                    }
                    format=MC_SCR_DATA_FORMAT_UNKNOWN;
                    if(e > 0)
                    {
                        tmpscript->SetElement(e-1);
                        tmpscript->GetDataFormat(&format);
                    }
                    elem=ptr;
                    elem_size=size;
                    out_size=elem_size;
    ...
    if( (format == MC_SCR_DATA_FORMAT_UBJSON) || (format == MC_SCR_DATA_FORMAT_UTF8) )
    {
        ...
        {
            *errCode=RPC_INVALID_PARAMETER;
            *strError="Invalid start, must be 0 for text and JSON data";                                        
            return 0;
        }
        if(count != out_size)
        {
            *errCode=RPC_INVALID_PARAMETER;
            *strError="Invalid count, must include all text or JSON data";                                        
            return 0;
        }
    }
    ...
    if(fHan)
    {
        if(chunk_count > 1)
        {
            if(elem == NULL)
            {
                elem=GetChunkDataInRange(rpc_slot,&out_size,chunk_hashes,chunk_count,start,count,fHan);
                if(elem == NULL)
                {
                    *errCode=RPC_INTERNAL_ERROR;
                    *strError="Couldn't retrieve data for this output";                                        
                    return 0;
                }
                return count;
            }
    ...
    if(chunk_count > 1)
    {
        if(elem == NULL)
        {
            elem=GetChunkDataInRange(rpc_slot,&out_size,chunk_hashes,chunk_count,start,count,0);
            if(elem == NULL)
            {
                *errCode=RPC_INTERNAL_ERROR;
                *strError="Couldn't retrieve data for this output";                                        
...
Value txouttobinarycache(const Array& params, bool fHelp)
{
    ...
    size=txoutdata_operation(GetRPCSlot(),ext_params,fHan,&errCode,&strError).get_int64();    
...
Value gettxoutdata(const Array& params, bool fHelp)
{
    ...
    pwalletTxsMain->WRPReadLock();
    Value ret= txoutdata_operation(GetRPCSlot(),params,0,&errCode,&strError);
    pwalletTxsMain->WRPReadUnLock();
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need accounts, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }            
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getbalance, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    string strAccount = AccountFromValue(params[0]);
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need move, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendfrom, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendmany, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
Value listtransactions(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need listtransactions, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    else
    {
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need listaccounts, run meliponariod -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }   
