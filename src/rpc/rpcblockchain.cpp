...
/* MCHN START */
bool ParseBitchainTxOutToBuffer(uint256 hash,const CTxOut& txout,mc_Buffer *amounts,mc_Script *lpScript,int *allowed,int *required,string& strFailReason);
...
//        throw JSONRPCError(RPC_INVALID_PARAMS, "Wrong number of parameters");                    
    ...
    if( (vin < 0) || (vin >= (unsigned int)pMeliponariodFilterEngine->m_Tx.vin.size()) )
    {
    ...
    Array getxoutparams;
    getxoutparams.push_back(pMeliponariodFilterEngine->m_Tx.vin[vin].prevout.hash.ToString());
    getxoutparams.push_back((int64_t)pMeliponariodFilterEngine->m_Tx.vin[vin].prevout.n);
...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    {
        if (!((CCoinsViewCache*)(pMeliponariodFilterEngine->m_CoinsCache))->GetCoins(hash, coins))
            return Value::null;        
    }
    else
    {
        if (fMempool) {
            ...
            if (!view.GetCoins(hash, coins))
                return Value::null;
            if(pMeliponariodFilterEngine->InFilter() == 0)                                // In filter we already checked this input exists, but mempool is dirty
            {
