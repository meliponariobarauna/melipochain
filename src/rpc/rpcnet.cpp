...
using namespace std;
void PushMeliponariodRelay(CNode* pto, uint32_t msg_type,vector<CAddress>& path,vector<CAddress>& path_to_follow,vector<unsigned char>& payload);
...
//                    PushMeliponariodRelay(pnode, MC_RMT_GLOBAL_PING,path,path_to_follow,payload);
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolBitchain())
    {
        obj.push_back(Pair("subversion",
            FormatSubVersion("Meliponariod", mc_gState->GetProtocolVersion(), std::vector<string>())));
    }
    else
    {
        obj.push_back(Pair("subversion",
