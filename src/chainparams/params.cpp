...
int mc_OneBitchainParam::IsRelevant(int version)
{
    int ret=1;
...
void mc_BitchainParams::Destroy()
{
    if(m_lpData)
    {
        mc_Delete(m_lpData);
...
int64_t mc_BitchainParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_BitchainParams::Int64ToDecimal(int64_t value)
{
    if(value < 0)
    {
...
int64_t mc_BitchainParams::DecimalToInt64(double value)
{
    return (int64_t)(value*MC_PRM_DECIMAL_GRANULARITY+mc_gState->m_NetworkParams->ParamAccuracy());    
}

int mc_BitchainParams::GetParamFromScript(char* script,int64_t *value,int *size)
{
    char *ptr;    
    ptr=script;
    ptr+=strlen(ptr)+1;
...
double mc_BitchainParams::GetDoubleParam(const char *param)
{
...
void* mc_BitchainParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_BitchainParams::IsParamUpgradeValueInRange(const mc_OneBitchainParam *param,int version,int64_t value)
{
...
int mc_BitchainParams::CanBeUpgradedByVersion(const char *param,int version,int size)
{
    if(m_lpIndex == NULL)
    {
...
int mc_BitchainParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_BitchainParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc_BitchainParams::Init()
{
    ...
    m_Count=sizeof(BitchainParamArray)/sizeof(mc_OneBitchainParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((BitchainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(BitchainParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(BitchainParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_OneBitchainParam*)mc_New(sizeof(BitchainParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,BitchainParamArray,sizeof(BitchainParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_BitchainParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_OneBitchainParam *param;
    char *ptrData;
    int num_sets;
...
double mc_BitchainParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


const mc_OneBitchainParam *mc_BitchainParams::FindParam(const char* param)
{
    int i;
    for(i=0;i<m_Count;i++)
    {
        if(strcmp((BitchainParamArray+i)->m_Name,param) == 0)
        {
            return BitchainParamArray+i;
        }        
    }    
    return NULL;
}

int mc_BitchainParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_BitchainParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc_BitchainParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc_BitchainParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((BitchainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((BitchainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (BitchainParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (BitchainParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((BitchainParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((BitchainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((BitchainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_BitchainParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_BitchainParams::Clone(const char* name, mc_BitchainParams* source)
{
    int err;
...
int mc_BitchainParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_BitchainParams::Validate()
{
    int i,size,offset;
    int isGenerated;
    int isMinimal;
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"meliponario") == 0)
            {
...
int mc_BitchainParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== Meliponariod configuration file ====\n\n");
    fprintf(fileHan,"# Created by meliponario-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"meliponariod %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun meliponario-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"meliponariod %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"meliponariod %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                ...
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence meliponario network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"meliponario-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running meliponariod for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"meliponario-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by meliponario-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by meliponariod.\n");
...
int mc_BitchainParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_BitchainParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_BitchainParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_BitchainParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_BitchainParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_BitchainParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_BitchainParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_BitchainParams::ProtocolVersion()
{
...
int mc_BitchainParams::IsProtocolBitchain()
{
    return m_IsProtocolMeliponariod;
}
