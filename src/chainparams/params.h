...
#ifndef MELIPONARIOPARAMS_H
#define	MELIPONARIOPARAMS_H
...
typedef struct mc_OneBitchainParam
{    
...
typedef struct mc_BitchainParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_OneBitchainParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolMeliponariod;
    int m_ProtocolVersion;
    ...
    int Clone(const char *name,mc_BitchainParams *source);
    ...
    const mc_OneBitchainParam *FindParam(const char *param);
    ...
    int IsParamUpgradeValueInRange(const mc_OneBitchainParam *param,int version,int64_t value);
