...
uint64_t nLocalMeliponariodServices = 0;
...
/* MCHN START */
    ...
    nLocalMeliponariodServices |= pEF->NET_Services();
    if(mc_gState->m_NetworkParams->IsProtocolBitchain())
    {
        subver=FormatSubVersion("Meliponariod", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, subver, nBestHeight, true, nLocalMeliponariodServices);
/* MCHN END */
}
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Meliponariod " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Meliponariod Core is probably already running."), addrBind.ToString());
        else
