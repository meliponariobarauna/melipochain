...
#include "filters/meliponariofilter.h"

extern mc_MeliponariodFilterEngine* pMeliponariodFilterEngine;
...
typedef struct CMeliponariodTxDetails
{    
...
void CMeliponariodTxDetails::Zero()
{
...
bool CMeliponariodTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool CMeliponariodTxDetails::IsRelevantEntity(uint160 hash)
{
...
void CMeliponariodTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
void CMeliponariodTxDetails::SetAffectedLibrary(void *entity)
{
    uint160 hash=0;
    memcpy(&hash,entity,MC_AST_SHORT_TXID_SIZE);
...
bool mc_CompareAssetQuantities(CMeliponariodTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void MeliponariodTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool MeliponariodTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool MeliponariodTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               CMeliponariodTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool MeliponariodTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       CMeliponariodTxDetails *details,           // Tx details object
...
bool MeliponariodTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          CMeliponariodTxDetails *details,   
                                          string& reason)      
{
...
bool MeliponariodTransaction_CheckCachedScript(const CTransaction& tx, 
...
void MeliponariodTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        CMeliponariodTxDetails *details)      
{
...
bool MeliponariodTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool MeliponariodTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               CMeliponariodTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!MeliponariodTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!MeliponariodTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!MeliponariodTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            MeliponariodTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool MeliponariodTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   CMeliponariodTxDetails *details,   
                                                   string& reason)      
{
...
bool MeliponariodTransaction_CheckVariableUpdateDetails(const CTransaction& tx,
                                                   mc_EntityDetails *entity,
                                                   int offset, 
                                                   int vout,
                                                   CMeliponariodTxDetails *details,   
                                                   string& reason)      
{
    ...
    int extended_script_row;                                                    
    uint32_t new_entity_type;                                                   
    int64_t total,last_total,left_position;
    int32_t chain_size;
    ...
    {
        reason="Metadata script rejected - variables not supported in this protocol version";
        return false;        
    }
 ...
 */ 
        ...
        if(err == 0)
        {
            if(bytes)
            {
                if(mc_gState->m_Features->ExtendedEntityDetails())
                {
                    extended_script_row=mc_gState->m_Assets->m_ExtendedScripts->GetNumElements();
                    mc_gState->m_Assets->m_ExtendedScripts->AddElement();
                    mc_gState->m_Assets->m_ExtendedScripts->SetData(ptr,bytes);
                }                
            }
    ...
    mc_gState->m_TmpScript->SetSpecialParamValue(MC_ENT_SPRM_VOUT,(unsigned char*)&vout,sizeof(vout));                            
...
bool MeliponariodTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                CMeliponariodTxDetails *details,   
...
bool MeliponariodTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           CMeliponariodTxDetails *details,   
                                           string& reason)      
{
...
bool MeliponariodTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           CMeliponariodTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!MeliponariodTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
       ...
       (entity.GetEntityType() == MC_ENT_TYPE_LIBRARY))                         // Asset, variable or library update
    {
        if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
        {
            if(!MeliponariodTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
            {
                return false;            
            }
        }
        ...
        {
            if(!MeliponariodTransaction_CheckVariableUpdateDetails(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
    ...
    else
    {
        ...
        {
            if(!MeliponariodTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!MeliponariodTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool MeliponariodTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             CMeliponariodTxDetails *details,   
                                             string& reason)      
{
...
bool MeliponariodTransaction_CheckIfLibraryApproval(unsigned char* ptr,
                                                  CMeliponariodTxDetails *details)
{
    mc_EntityDetails entity;
...
bool MeliponariodTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
    ...
    for (int e = 0; e < mc_gState->m_TmpScript->GetNumElements(); e++)
    {
        ...
        {   
            if(mc_gState->m_TmpScript->GetPermission(&type,&from,&to,&timestamp) == 0) // Grant script
            {
                ...
                if(fFirstPass)  
                {
                    if( type & ( MC_PTP_CREATE | MC_PTP_ISSUE | MC_PTP_ACTIVATE | MC_PTP_FILTER | MC_PTP_DETAILS ) )
                    {
                            ...
                            if(details->vInputDestinations[i] != 0)
                            {
                                if( ( (type & (MC_PTP_ADMIN | MC_PTP_MINE)) == 0) || details->vInputCanGrantAdminMine[i] || (entity.GetEntityType() > 0) )
                                {
                                            ...
                                            from,to,timestamp,flags,1,offset) == 0)
                                    {
                                        if(type & MC_PTP_FILTER)
                                        {
                                            MeliponariodTransaction_CheckIfLibraryApproval(ptr,details);
                                        }
                                        fAdminFound=true;
                                    }
                                }
...
bool Is_MeliponariodLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    MeliponariodTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool MeliponariodTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               CMeliponariodTxDetails *details,   
                                               string& reason)      
{
...
bool MeliponariodTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               CMeliponariodTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!MeliponariodTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool MeliponariodTransaction_ProcessTokenIssuance(const CTransaction& tx,
                                                int offset,  
                                                bool accept,
                                                int vout,
...
{    
    ...
    for (int e = 0; e < mc_gState->m_TmpScript->GetNumElements(); e++)
    {
        ...
        else
        {
            if(err != MC_ERR_WRONG_SCRIPT)
            {
                reason="Token issuance script rejected - error in script";
                return false;                                    
            }
        }                
    }
    ...
    for (int e = 0; e < mc_gState->m_TmpScript->GetNumElements(); e++)
    {
        ...
        if((err != MC_ERR_NOERROR) && (err != MC_ERR_WRONG_SCRIPT))
        {
            reason="Token issuance script rejected - error in token script";
            return false;                                
        }
    }
    ...
    if(token_details_element < 0)
    {
        reason="Token issuance script rejected - missing inline details";
        return false;                                                            
    }
        
    if(mc_gState->m_TmpAssetsOut->GetCount() > 1)
    {
        reason="Token issuance script rejected - multiple token scripts";
        return false;                                                                    
    }
    ...
    if(new_issue)
    {
        ...
        {
            reason="Token issuance script rejected - token script in asset issuance output";
            return false;                                                                    
        }
    }
    else
    {
        vout_total=mc_GetABQuantity(mc_gState->m_TmpAssetsOut->GetRow(0));
    }
    ...
    if(param_offset<(uint32_t)token_details_size)
    {
        ...
        if(token_id_size == 0)
        {
            reason="Token issuance script rejected - empty token identifier";
            return false;                                                                        
        }
    }
    else
    {
        reason="Token issuance script rejected - missing token identifier";
        return false;                                                                                
    }
    ...
    if(issue_txid == 0)
    {
        ...
        if(param_offset<(uint32_t)token_details_size)
        {
            ...
            {
                reason="Token issuance script rejected - wrong asset txid size";
                return false;                                                                                       
            }
            memcpy(&issue_txid,token_details+value_offset,value_size);
        }
        else
        {
            reason="Token issuance script rejected - missing asset txid";
            return false;                                                                                
        }        
    }

    if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&issue_txid) == 0)
    {
        reason="Token issuance script rejected - asset not found";
        return false;                                                                                
    }    
    
    if(entity.IsNFTAsset() == 0)
    {
        reason="Token issuance script rejected - not allowed for this asset";
        return false;                                                                                        
    }
    
    if(entity.AllowedFollowOns() == 0)
    {
        reason="Token issuance script rejected - followons not allowed for this asset";
        return false;                                                                                        
    }
    ...
    hasher->Write(token_id,token_id_size);
    ...
    if(vout_total+last_total < 0)
    {
        reason="Token issuance script rejected - exceeds maximal value for asset";
        return false;                                                                                                                
    }
    ...
    if(txid != 0)
    {
        mc_gState->m_TmpScript->SetSpecialParamValue(MC_ENT_SPRM_PARENT_ENTITY,(unsigned char*)&txid,sizeof(txid));                                    
    }
    ...
    if(err)
    {
        reason="Cannot update permission database for issued token";
        return false;            
    }
    ...
    if(err)           
    {
        reason="Token issue script rejected - could not insert asset followon to database";
        if(err == MC_ERR_FOUND)
        {
            reason="Token issue script rejected - entity with this name/asset-ref/txid already exists";                        
            if(fDebug)LogPrint("mchn","Entity already exists. TxID: %s \n",
                    this_txid.ToString().c_str());                                        
        }
        return false;                                            
    }
    ...
    if(err)           
    {
        reason="Token issue script rejected - could not insert token to database";
        if(err == MC_ERR_FOUND)
        {
            reason="Token issue script rejected - entity with this name/asset-ref/txid already exists";                        
            if(fDebug)LogPrint("mchn","Entity already exists. TxID: %s \n",
    ...
    if(update_mempool)
    {
        if(offset>=0)
        {
            if(mc_gState->m_Assets->FindEntityByTxID(&this_entity,(unsigned char*)&this_txid))
            {
                string token_id_str;
                token_id_str.resize(token_id_size+1,0x00);
                std::copy(token_id,token_id+token_id_size,begin(token_id_str));
                if(fDebug)LogPrint("mchn","New Token. Asset issue txid: %s, Token ID: %s\n",
                        issue_txid.ToString().c_str(),token_id_str.c_str());
                if(fDebug)LogPrint("mchn","TxID: %s,  Token hash: %s\n",
                        tx.GetHash().GetHex().c_str(),token_hash.ToString().c_str());
            }
            else
            {
                reason="Token issue script rejected - could not insert new asset to database";
                return false;                                            
            }
        }
    }
...
bool MeliponariodTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        MeliponariodTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!MeliponariodTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!MeliponariodTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!MeliponariodTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!MeliponariodTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!MeliponariodTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }                                                                                        
    ...
    if(mc_gState->m_Features->NFTokens())
    {
        for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
        {
            ...
            if(!MeliponariodTransaction_ProcessTokenIssuance(tx,offset,accept,vout,0,details,reason))
            {
                return false;                
            }        
        }    
...
bool MeliponariodTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        CMeliponariodTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!MeliponariodTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t MeliponariodTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool MeliponariodTransaction_CheckMandatoryFee(CMeliponariodTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = MeliponariodTransaction_OffchainFee(details->total_offchain_size);
...
bool MeliponariodTransaction_CheckLicenseTokenDetails(CMeliponariodTxDetails *details,     // Tx details object
...
bool MeliponariodTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
                                                ...
                                                string& reason)                 // Error message
{
    ...
    if(details->details_script_type == 0)                                       // New asset/variable/library with details script
    {
        ...
        value_offset=mc_FindSpecialParamInDetailsScript(details->details_script,details->details_script_size,MC_ENT_SPRM_FOLLOW_ONS,&value_size);
        if(value_offset<(uint32_t)details->details_script_size)
        {
            followon_mode=mc_GetLE(details->details_script+value_offset,value_size);
        }                                    
        value_offset=mc_FindSpecialParamInDetailsScript(details->details_script,details->details_script_size,MC_ENT_SPRM_ASSET_MAX_TOTAL,&value_size);
        if(value_offset<(uint32_t)details->details_script_size)
        {
...
//        if(details->new_entity_type == MC_ENT_TYPE_VARIABLE)                   
    ...
    if(new_issue)
    {
        if(mc_gState->m_Features->NFTokens())
        {
            if(followon_mode & MC_ENT_FOMD_NON_FUNGIBLE_TOKENS)
            {
                if(multiple != 1)
                {
                    reason="Asset issue script rejected - multiple should be 1 for non-fungible tokens asset";
                    return false;                                                                                                                
                }
                if((followon_mode & (MC_ENT_FOMD_ALLOWED_INSTANT | MC_ENT_FOMD_CAN_OPEN)) == 0)
                {
            ...
            (details->new_entity_type != MC_ENT_TYPE_LIBRARY) )
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    MeliponariodTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!MeliponariodTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
    ...
    if(!details->fLicenseTokenIssuance)
    {
        ...
        unsigned char issuer_buf[24];
        unsigned char details_address[20];
        memset(issuer_buf,0,sizeof(issuer_buf));
        flags=MC_PFL_NONE;        
    ...
    if(err == 0)
    {
        if(mc_gState->m_Features->NFTokens())
        {
            if(followon_mode & MC_ENT_FOMD_NON_FUNGIBLE_TOKENS)
            {
                ...
                {
                    MeliponariodTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);
                    if(!MeliponariodTransaction_ProcessTokenIssuance(tx,offset,accept,vout,txid,details,reason))
                    {
                        return false;                
                    }        
                }
...
bool MeliponariodTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 CMeliponariodTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                pMeliponariodFilterEngine->AddFilter((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool MeliponariodTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         CMeliponariodTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool MeliponariodTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        CMeliponariodTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocolBitchain() == 0))
    {
        LogPrintf("Non-standard coinbase: Sigature output doesn't match protocol\n");
        return true;                                                            // Signature output should appear only if protocol=meliponario
    }
...
bool MeliponariodTransaction_ProcessLibraryUpdates(int offset,
                                                 CMeliponariodTxDetails *details, // Tx details object                                                 
                                                 string& reason)                // Error message
{
    ...
    if(pMeliponariodFilterEngine)
    {    
        pMeliponariodFilterEngine->CheckLibraries(&(details->vAffectedLibraries),(offset >= 0));
    }
    return true;
}


bool AcceptMeliponariodTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    if(mc_gState->m_NetworkParams->IsProtocolBitchain() == 0)                 
    {
        return true;
    }    
    
    details.fCheckCachedScript=MeliponariodTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!MeliponariodTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!MeliponariodTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!MeliponariodTransaction_CheckOutputs(tx,inputs,offset,accept,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!MeliponariodTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!MeliponariodTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!MeliponariodTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!MeliponariodTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!MeliponariodTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!MeliponariodTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!MeliponariodTransaction_ProcessLibraryUpdates(offset,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
        ...
        ((flags & MC_AMT_NO_FILTERS) == 0) )
    {
        if(mc_gState->m_Features->Filters())
        {
            if(pMeliponariodFilterEngine)
            {
                mc_MeliponariodFilter* lpFilter;
                int applied=0;

                if(pMeliponariodFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
...
exitlbl:
    ...
    if(!accept || fReject)                                                      // Rolling back permission database if we were just checking or error occurred    
    {
        ...
        MeliponariodTransaction_ProcessLibraryUpdates(offset,&details,reason);
    }
