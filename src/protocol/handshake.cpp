...
bool BitchainNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !BitchainNode_CanConnect(pnode);
    }
...
bool BitchainNode_AcceptData(CNode *pnode)
{
    ...
    return !BitchainNode_DisconnectRemote(pnode);
}
...
bool VerifyBitchainVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyBitchainVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!BitchainNode_CanConnect(pfrom))
            {
