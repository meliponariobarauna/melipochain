...
mc_MeliponariodFilterEngine* pMeliponariodFilterEngine = NULL;
...
/* MCHN START */  
    ...
    {
        delete pMeliponariodFilterEngine;
        pMeliponariodFilterEngine=NULL;        
    }
...
std::string HelpMessage(HelpMessageMode mode)                                   // MCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "meliponario.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "meliponario.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default meliponario address otherwise") + "\n";
#endif
/* MCHN START */    
/* Default was 0 */    
...
//    strUsage += "  -blockprioritysize=<n> " + strprintf(_("Set maximum size of high-priority/low-fee transactions in bytes (default: %d)"), DEFAULT_BLOCK_PRIORITY_SIZE) + "\n";
    ...
    strUsage += "  -healthcheckport=<port>" + _("Listen for health checker connections on <port>, Enterprise Edition only.") + "\n";
    ...
    strUsage += "\n" + _("Meliponariod runtime parameters") + "\n";    
    strUsage += "  -offline                                 " + _("Start meliponariod in offline mode, no connections to other nodes.") + "\n";
    strUsage += "  -storeruntimeparams                      " + _("Permanently save modifications to runtime parameters made by setruntimeparam APIs.") + "\n";
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running meliponariod for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a Meliponariod blockchain.") + "\n";
    ...
    strUsage += "  -explorersupport=0|2                     " + _("Provide support for Meliponariod Explorer 2, default 0") + "\n";

    strUsage += "\n" + _("Meliponariod API response parameters") + "\n";        
    strUsage += "  -hideknownopdrops      " + strprintf(_("Remove recognized Meliponariod OP_DROP metadata from the responses to JSON-RPC calls (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -v1apicompatible       " + strprintf(_("JSON-RPC calls responses compatible with Meliponariod 1.0 (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) Coin Sciences Ltd - www.meliponario.com")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.meliponario.com/terms-of-service/")) +
           "\n";
        ...
        if(pwalletMain)
        {
            if(pwalletMain->vchDefaultKey.IsValid())
            {
                ...
                {    
                    ...
                    sprintf(bufOutput,"meliponario-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                         CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                    sprintf(bufOutput,"meliponario-cli %s grant %s connect,send,receive\n\n",mc_gState->m_NetworkParams->Name(),
                         CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                }
                else
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Meliponariod Core is shutting down."));
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Meliponariod Core is probably already running."), strDataDir));
#endif
/* MCHN END */
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocolBitchain())
    {
        LogPrintf("Meliponariod version build %s protocol %s (%s)\n", mc_BuildDescription(mc_gState->GetNumericVersion()), mc_gState->GetProtocolVersion(), CLIENT_DATE);
    }
    ...
    if (!fDisableWallet) {
        ...
        if (filesystem::exists(pathWalletDat))
        {
            ...
            if(currentwalletdatversion != 1)
            {
                ...
                {
                    return InitError(strprintf("Wallet version %d is not supported in this edition of Meliponariod.\n"
                            "To upgrade to version %d, run Meliponariod Offline Daemon: \n"
                            "meliponariod-cold %s -datadir=%s -walletdbversion=3\n"
                            "and restart meliponariod or use Community Edition.\n",
        ...
        switch(currentwalletdatversion)
        {
            ...
            case 2:
                break;
            case 1:
                return InitError(strprintf("Wallet version 1 is not supported in this version of Meliponariod. "
                        "To upgrade to version 2, run Meliponariod 1.0: \n"
                        "meliponariod %s -walletdbversion=2 -rescan\n",mc_gState->m_NetworkParams->Name()));                                        
            default:
...
//            if (!CWalletDB::Recover(bitdbwrap, strWalletFile, true))
            if(!WalletDBRecover(bitdbwrap,strWalletFile,true))
                return false;
            sprintf(bufOutput,"\nTo work properly with salvaged addresses, you have to call importaddress API and restart Meliponariod with -rescan\n\n");
...
//        uiInterface.InitMessage.connect(SetRPCWarmupStatus);
    ...
    uiInterface.InitMessage(_("Initializing meliponario..."));
    ...
    {    
        ...
        if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
        {
            ...
            return InitError(_("wallet.dat is partially corrupted. Please try running Meliponariod with -salvagewallet."));                            
        }
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to Meliponariod 1.0 beta 2 or later"));                                                        
                        }
...
*/
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_resolved.c_str()))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart meliponariod";
...
//                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d - please check meliponariod is running at that address and that your firewall settings allow incoming connections.",                
                            seed_ip.c_str(),seed_port);
                        pNodeStatus->sLastError="Couldn't connect to the seed node, meliponariod is not running or firewall issue";
                    }
                }
            }
            else
            ...
            if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
            {
                ...
                if(mc_gState->IsSupported(protocol_version) == 0) 
                {
                    ...
                    {
                        seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in Meliponariod %s\n",                
                    ...
                    else
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d.\n"
                                    "Blockchain was created by meliponariod with newer protocol version (%d)\n"                
                                    "Please upgrade to the latest version of Meliponariod or connect only to blockchains using protocol version %d or earlier.\n",                
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
            {
                SelectMeliponariodParams(mc_gState->m_Params->NetworkName());
        ...
        else
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_GENERATED)
            {
                ...
                mc_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocolBitchain flag in case of bitcoin
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart meliponariod with reindex=1.\n",mc_gState->m_Params->NetworkName());
...
*/
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart meliponariod with -rescan\n");                        
                }
                if((pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_EXPLORER_MASK) == MC_WMD_EXPLORER1)
                {
                    return InitError("-explorersupport=1 is not supported by this version of Meliponariod. To change it, please restart meliponariod with -rescan -explorersupport=2\n");                                                                                
                }
                if(explorer_mode >= 0)
                {                    
                    if( (pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_EXPLORER_MASK) != (mc_gState->m_WalletMode & MC_WMD_EXPLORER_MASK) )
                    {
                        return InitError("Explorer database was initialized with different setting. To change it, please restart meliponariod with -rescan\n");                                                    
                    }
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart meliponariod with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        
                ...
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart meliponariod with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after meliponario parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
            ...
            if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
            {
                ...
                return InitError(_("wallet.dat corrupted. Please try running Meliponariod with -salvagewallet."));                            
            }
...
/* MCHN START */    
    ...
    pMeliponariodFilterEngine=new mc_MeliponariodFilterEngine;
    if(pMeliponariodFilterEngine->Initialize())
    {
    ...
    if(!GetBoolArg("-shortoutput", false))
    {
        if(fListen && !GetBoolArg("-offline",false))
        {
            ...
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            sprintf(bufOutput,"meliponariod %s:%d\n\n",BitchainServerAddress(false).c_str(),GetListenPort());
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            if(found_ips > 1)
            {
        ...
        else
        {
            if(GetBoolArg("-offline",false))
            {                
                sprintf(bufOutput,"Meliponariod started in offline mode, other nodes cannot connect.\n\n");
                bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));            
            }
            else
            {
    ...
    {
        sprintf(bufOutput,"%s:%d\n",BitchainServerAddress(true).c_str(),GetListenPort());                
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
    }
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart meliponariod with reindex=1."),
...
//    int version=mc_gState->m_NetworkParams->GetInt64Param("protocolversion");
    ...
    LogPrintf("Meliponariod protocol version: %d\n",version);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Meliponariod Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart Meliponariod Core to complete") << "\n";
