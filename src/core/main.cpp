...
bool ProcessBitchainRelay(CNode* pfrom, CDataStream& vRecv, CValidationState &state);
bool ProcessBitchainVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushMeliponariodVerack(CNode* pfrom, bool fIsVerackack);
bool BitchainNode_CanConnect(CNode *pnode);
bool BitchainNode_DisconnectRemote(CNode *pnode);
bool BitchainNode_DisconnectLocal(CNode *pnode);
bool BitchainNode_RespondToGetData(CNode *pnode);
bool BitchainNode_SendInv(CNode *pnode);
bool BitchainNode_AcceptData(CNode *pnode);
bool BitchainNode_IgnoreIncoming(CNode *pnode);
bool BitchainNode_IsLocal(CNode *pnode);
bool BitchainNode_CollectChunks();
...
const string strMessageMagic = "Meliponariod Signed Message:\n";
...
int SetUpgradedParamValue(const mc_OneBitchainParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        SetMeliponariodParam("targetblocktime",value);
    }   
...
int BitchainNode_ApplyUpgrades(int current_height)
{
    ...
    SetMeliponariodParams();            
    if(pMeliponariodFilterEngine)                                                 // Added from version 20012. 
    {
        pMeliponariodFilterEngine->SetCallbackNames();
    }
...
void BitchainNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool BitchainNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(!AcceptMeliponariodTransaction(tx,view,-1,MC_AMT_DEFAULT,reason, &mandatory_fee, &replay))
        {
            return state.DoS(0,
                             error("AcceptToMemoryPool: : AcceptMeliponariodTransaction failed %s : %s", hash.ToString(),reason),
                             REJECT_NONSTANDARD, reason);
        }
...
*/
        ...
        CAmount nFees = 0;
        int async_count=0;
        bool fIsLicenseTokenTransfer=Is_MeliponariodLicenseTokenTransfer(tx);
...
/* MCHN START */        
    if(!fJustCheck)
    {
        ...
        {
            pMeliponariodFilterEngine->Reset(pindex->nHeight-1,1);
        }
    }
/* MCHN END */        
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            const CTransaction &tx = block.vtx[i];
            string reason;
            if(!AcceptMeliponariodTransaction(tx,view,offset,MC_AMT_DEFAULT,reason,NULL,NULL))
            {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                ...
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptMeliponariodTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
                }
            }
/* MCHN END */                    
...
/* MCHN START */        
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(!AcceptMeliponariodTransaction(tx,view,coinbase_offset,MC_AMT_DEFAULT,reason,NULL,NULL))
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : AcceptMeliponariodTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
...
/* MCHN START */    
    ...
    if(pMeliponariodFilterEngine)
    {
        pMeliponariodFilterEngine->Reset(old_height-1,0);
    }
    
    BitchainNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/*    
    if(pMeliponariodFilterEngine)
    {
        pMeliponariodFilterEngine->Reset(pindexNew->nHeight-1,1);
    }
 */ 
    ...
    }
    {
        ...
        if(pMeliponariodFilterEngine)
        {
            pMeliponariodFilterEngine->m_CoinsCache=&view;
        }
        bool rv = ConnectBlock(*pblock, state, pindexNew, view);        
        if(pMeliponariodFilterEngine)
        {
            pMeliponariodFilterEngine->m_CoinsCache=NULL;
        }
        if(rv)
        {
...
/* MCHN START */        
    ...
    BitchainNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
//    if (!fInvalidFound)
    {
        ...
        if(pMeliponariodFilterEngine)
        {
            pMeliponariodFilterEngine->Reset(chainActive.Height(),0);
        }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    BitchainNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            BitchainNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */
    ...
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
        ...
        if(pwalletTxsMain->RollBack(NULL,chainActive.Height()) != MC_ERR_NOERROR)
        {
            ...
            {
                strError="Error: The wallet database is inconsistent. Restart Meliponariod with -rescan to rebuild the wallet database from the blockchain (can take hours), or with -skipwalletchecks to continue operating anyway";
                return false;
            }
        }
    }
...
/* MCHN END */        
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if(pMeliponariodFilterEngine)
            {
                pMeliponariodFilterEngine->m_CoinsCache=&coins;
            }            
...
/* MCHN START */    
    if(!BitchainNode_RespondToGetData(pfrom)) 
    {
...
//    RandAddSeedPerfmon();
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->nMeliponariodServices=0;
        if (!vRecv.empty())
        {
            vRecv >> pfrom->nMeliponariodServices;
        }
...
/* MCHN START */
        ...
        {            
            ...
            {
                PushMeliponariodVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolBitchain())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessBitchainVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushMeliponariodVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolBitchain())
        {
            ...
            if(!ProcessBitchainVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushMeliponariodVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!BitchainNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !BitchainNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && BitchainNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!BitchainNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && BitchainNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && BitchainNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(BitchainNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && BitchainNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !BitchainNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(BitchainNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=BitchainNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && BitchainNode_IsBlockChainSynced(pto))
            {
...
/* MCHN START */        
        ...
        if(pwalletTxsMain->m_ChunkCollector)
        {
            ...
            {
                if(BitchainNode_CollectChunks())
                {
                    BitchainCollectChunks(pwalletTxsMain->m_ChunkCollector);
                }                
                pwalletTxsMain->m_ChunkCollector->m_NextTryTimestamp=time_millis_now+BitchainCollectChunksQueueStats(pwalletTxsMain->m_ChunkCollector);
