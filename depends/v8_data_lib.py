...
def get_options():
    mc_path_default = str(Path.home() / "meliponario")
    ...
    parser.add_argument("-m", "--meliponario", metavar="DIR", default=mc_path_default,
                        help="Meliponariod path prefix (default: %(default)s)")
    ...
    if not Path(options.meliponario).exists():
        parser.error("{!r}: Meliponariod path does not exist".format(options.meliponario))

    logger.info("{} - {}".format(logger.name, parser.description))
    logger.info("  Path:     {}".format(options.meliponario))
...
def main():
    ...
    options = get_options()
    os.chdir(str(Path(options.meliponario) / "v8build" / "v8" / "out.gn" / "x64.release"))
